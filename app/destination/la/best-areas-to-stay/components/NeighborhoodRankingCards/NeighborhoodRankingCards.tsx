            ...
            <div key={n.id} id={n.id} className={styles.card}>
              ...
              <div className={styles.content}>
                ...
                <div className={styles.tags}>
                  {n.bestFor.map((tag) => <span key={tag} className={styles.tag}>{tag}</span>)}
                </div>
                <div className={styles.columns}>
                  <div><h4>Pros</h4><ul>{n.pros.map((p) => <li key={p}>{p}</li>)}</ul></div>
                  <div><h4>Cons</h4><ul>{n.cons.map((c) => <li key={c}>{c}</li>)}</ul></div>
                </div>
