...
export default function FemaleSoloTiles({ tiles, ctaLink, cityName = "LA" }: Props) {
  return (
      ...
      <div className={styles.grid}>
        {tiles.map((tile, index) => (
          <div key={index} className={styles.card}>
            <div className={styles.icon}>{iconMap[tile.icon] ?? null}</div>
            <h3 className={styles.title}>{tile.title}</h3>
            <p className={styles.description}>{tile.description}</p>
            <Link href={tile.link} className={styles.link}>Learn More →</Link>
          </div>
        ))}
      </div>
      <div className={styles.ctaWrapper}>
        <Link href={ctaLink} className={styles.cta}>Solo Female Guide →</Link>
      </div>
    </section>
  );
}
