...
export default function ThreeDayItinerary({ days, ctaLink, cityName = "LA" }: ItineraryProps & { cityName?: string }) {
  return (
        ...
        {days.map((day, index) => (
          <div key={index} className={styles.dayBlock}>
            ...
            <ul className={styles.bullets}>
              {day.bullets.map((item, i) => <li key={i}>{item}</li>)}
            </ul>
          </div>
        ))}
      </div>
      <div className={styles.ctaWrapper}>
        <Link href={ctaLink} className={styles.cta}>Full 3‑Day Itinerary →</Link>
      </div>
    </section>
  );
}
