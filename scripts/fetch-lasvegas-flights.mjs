#!/usr/bin/env node
/**
 * Fetch cheapest fares on popular US routes TO Las Vegas (LAS) from the
 * Travelpayouts Aviasales v3 "prices_for_dates" API, then rewrite
 * content/cities/lasvegas/bookflights.json (powers /destination/lasvegas/bookings).
 *
 * Usage:
 *   node scripts/fetch-lasvegas-flights.mjs
 *
...
const JSON_PATH = path.join(ROOT, "content", "cities", "lasvegas", "bookflights.json");
...
// Popular US origins -> Las Vegas (LAS)
const ORIGINS = {
  LAX: "Los Angeles (LAX)",
...
const AIRPORT_NAMES = {
  LAX: "Los Angeles (LAX)", SFO: "San Francisco (SFO)", SEA: "Seattle (SEA)",
  PHX: "Phoenix (PHX)", DEN: "Denver (DEN)", ORD: "Chicago (ORD)",
  ATL: "Atlanta (ATL)", DFW: "Dallas (DFW)", DAL: "Dallas (DAL)",
  MIA: "Miami (MIA)", JFK: "New York (JFK)", LGA: "New York (LGA)",
  EWR: "Newark (EWR)", BOS: "Boston (BOS)",
};
...
function buildRow(item, prev) {
  ...
  const destAirport = item.destination_airport || "LAS";
  return {
    airline: {
...
async function main() {
  console.log("Fetching cheapest fares to Las Vegas (LAS)...");
  const results = [];
  for (const origin of Object.keys(ORIGINS)) {
    const item = await cheapestFor(origin);
    if (item) {
      console.log(`  ${origin} -> LAS: $${Math.round(item.price)} (${item.airline})`);
      results.push(item);
    }
  }
  if (!results.length) {
