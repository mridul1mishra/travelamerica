#!/usr/bin/env node
/**
 * Fetch "Things to Do" in Las Vegas from the WeGoTrip API (a Travelpayouts
 * tours & activities partner) and write content/cities/lasvegas/thingstodo.json
 * (Things to Do tab on /destination/lasvegas/bookings).
 *
 * Usage:
 *   node scripts/fetch-lasvegas-thingstodo.mjs
 *
 * Env (loaded from .env.local / .env):
 *   WEGOTRIP_API            - optional, default https://app.wegotrip.com/api/v2
 *   WEGOTRIP_LV_CITY_ID     - optional, default 5506956 (Las Vegas, GeoNames)
...
const JSON_PATH = path.join(ROOT, "content", "cities", "lasvegas", "thingstodo.json");
...
const CITY_ID = process.env.WEGOTRIP_LV_CITY_ID || "5506956"; // Las Vegas (GeoNames)
const MARKER = process.env.TRAVELPAYOUTS_MARKER || "";
const LIMIT = Number(process.env.THINGSTODO_LIMIT || 12);

const AFFILIATE_PATH = path.join(ROOT, "content", "cities", "lasvegas", "thingstodo.affiliate.json");
let AFFILIATE = {};
try {
...
function productUrl(item) {
  ...
  const citySlug = item?.city?.slug || "las-vegas";
...
function toCard(item) {
  ...
  return {
    img: item?.preview || item?.cover || "/data/majorcities/lasvegas/assets/hotel.svg",
...
async function main() {
  console.log(`Fetching Las Vegas things to do from WeGoTrip (${BASE}, city ${CITY_ID})...`);
