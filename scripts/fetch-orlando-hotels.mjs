#!/usr/bin/env node
/**
 * Fetch Orlando hotels from the RapidAPI "Travel Advisor" API and write
 * content/cities/orlando/hotels.json (Hotels tab on /destination/orlando/bookings).
 *
 * Usage:
 *   node scripts/fetch-orlando-hotels.mjs
 *
...
const JSON_PATH = path.join(ROOT, "content", "cities", "orlando", "hotels.json");
...
const ORLANDO_LOCATION_ID = "34515"; // Orlando, FL (Tripadvisor geo id)
...
async function getLocationId() {
  ...
  console.warn(`  falling back to known Orlando location_id ${ORLANDO_LOCATION_ID}`);
...
function toCard(item) {
  const img =
    ...
    "/data/majorcities/orlando/assets/holiday-inn.jpg";
  const rating = num(item?.rating) ?? num(item?.bubble_rating?.rating);
  const reviews = num(item?.num_reviews);
  const area = item?.parent_display_name || item?.location_string || "Orlando, FL";
  return {
    img,
    title: item.name,
    area,
    ...
    url:
      item?.web_url ||
      `https://www.google.com/travel/hotels/${encodeURIComponent(item.name + " Orlando")}`,
  };
}

async function main() {
  console.log(`Fetching Orlando hotels via RapidAPI (${HOST}) for check-in ${CHECK_IN}...`);
  const locationId = await getLocationId();
  console.log(`  Orlando location_id = ${locationId}`);
